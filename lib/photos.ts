 ...
 * artwork behind this map any more: a key with no photograph renders as an
 * empty plate saying so, which is why every listing in `lib/homes.ts`
 * declares only the scenes it actually has a picture of. Add a scene to a
 * listing without adding its photograph here and the gallery will show that
 * plate — check both together.
 *
 * READ THIS BEFORE THE SITE GOES LIVE. Two kinds of photograph are mixed
 * below, and they are not equally true:
 *
 *   Clayton's own model photography, read off the model page each listing
 *   links as its `sourceUrl` — buttercup, cook, crockett, el-sueno-breeze,
 *   farm-4-flex-elite, franklin, haven, magellan, the-fusion-32b and
 *   willow. These picture the plan they are attached to.
 *
 *   Stand-in photography — anniversary-choice, breeze, cypress,
 *   double-maxx-elite-56, double-maxx-elite-64, elation, everett, pride,
 *   satisfaction and tinsley. These are pictures of other manufactured
 *   homes on a dealer's lot, matched to each listing by section count, size
 *   and scene set. They are photographs of the type of home, not of the
 *   plan named on the listing, and the footer tells a visitor so.
 *
 * Replace the second group with photographs of the actual home before
