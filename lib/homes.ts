...
/* ------------------------------------------------------------------ *
 * The catalogue — twenty Clayton-built plans
 *
 * Clayton is all we carry, so every home below is a real Clayton plan,
 * across the brands Clayton builds under: TRU, TRU Mini, NXT, Epic,
 * Anniversary, the Patriot Collection, Ranger, Independent, Double Maxx,
 * Foundation and CrossMod. Names, model codes, bedroom and bathroom counts
 * and square footages come from the manufacturer's or a retailer's own
 * published sheet — `sourceUrl` on each listing is where it was read.
 *
 ...
 * a 28 × 56 three-bedroom). Where a plan's dimensions were not published,
 * the fields are absent rather than derived.
 *
 * No prices: Clayton publishes none uniformly and a dealer quotes on
 * options, delivery distance and site work, so the site says "call for
 * pricing" everywhere a price would go. Floor-plan geometry is likewise
 * absent — the drawings on this site are generated, and a generated layout
 * for a real plan would be wrong rather than merely missing.
 *
 * `status`, `featured`, `daysListed` and `communitySlug` are lot state for
 * this demonstration site, not manufacturer data.
 * ------------------------------------------------------------------ */
...
export const listings: Listing[] = [
  /* ---------------- Single-section ---------------- */
  {
    ...
    highlights: [
      "408 square feet — the smallest plan on the lot",
  ...
  },
  {
    ...
    featured: true,
    style: "farmhouse",
    communitySlug: "amherst-ridge",
    sourceUrl:
      "https://www.claytonhomes.com/homes-for-sale/manufactured-homes/anniversary-choice",
    tagline: "The 16 × 76 that outsells everything else in the state, with the island kitchen.",
    story: [
      "If you have walked one manufactured home in East Tennessee, it was probably this footprint. Sixteen by seventy-six, three bedrooms split with the primary at one end, a kitchen island in the middle of the home and the utility room off the back door.",
  ...
  },
  {
    ...
    tagline: "Two thousand one hundred and twenty-eight square feet, arriving on the lot next month.",
    scenes: scenes(
      ["exterior", "Front elevation"],
      ["living", "Great room"],
      ["kitchen", "Kitchen and island"],
